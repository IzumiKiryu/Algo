...
function bob(a,b,c) {
	let result=0;
	for(let i=0; i<a; i++) { //Répéter a fois ===> Triple imbrication simplifiée (Ajouter a*(b*c) à la variable result)
		for(let j=0; j<b; j++) { //Répéter b fois (Ajouter b*c à la variable result)
			for(let k=0; k<c; k++) { //Répéter 3 fois (c fois) (ou ajouter c à la variable result)
				result += 1; //Ajouter 1 à result
			}
		}
	}
	return result;
